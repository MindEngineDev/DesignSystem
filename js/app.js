...
export function setupTabs(rootSel) {
  ...
  const tabs = qsa('[data-tab]', root), panes = qsa('[data-pane]', root);
...
function serializeForm(form) {
  const data = {}, fm = new FormData(form);
  ...
  form.querySelectorAll("sl-input[name], sl-select[name], sl-textarea[name]").forEach(el => {
    ...
    if (name) try { data[name] = el.value; } catch { }
  });
  return data;
}
export function attachFormDemo(form) {
  ...
  fill.type = "button"; fill.className = "secondary"; fill.textContent = "Fill sample data";
  const clear = document.createElement("button");
  clear.type = "button"; clear.className = "contrast"; clear.textContent = "Clear";
  const copy = document.createElement("button");
  copy.type = "button"; copy.className = "secondary"; copy.textContent = "Copy JSON";
  controls.append(fill, clear, copy);
  const wrap = document.createElement("div"); wrap.className = "forms__demo";
  form.parentNode.insertBefore(wrap, form.nextSibling); wrap.append(controls, preview);
  const update = () => preview.textContent = JSON.stringify(serializeForm(form), null, 2);
  form.addEventListener("input", update); form.addEventListener("change", update);
  copy.addEventListener("click", async () => {
    try { await navigator.clipboard.writeText(preview.textContent); copy.textContent = "Copied"; setTimeout(() => copy.textContent = "Copy JSON", 900); } catch { }
  });
  fill.addEventListener("click", () => {
    form.querySelectorAll("input, textarea, select, sl-input, sl-textarea, sl-select").forEach(el => {
      ...
      else if (el.tagName.toLowerCase().startsWith("sl-")) { try { el.value = el.getAttribute("data-sample") || ("Sample " + name); el.dispatchEvent(new Event("sl-change")); } catch { } }
      else if (el.tagName.toLowerCase() === "select") { if (el.options?.length) el.selectedIndex = 0; }
      else if (el.type !== "file") el.value = el.getAttribute("placeholder") || ("Sample " + name);
    }); update();
  });
  clear.addEventListener("click", () => { form.reset(); form.querySelectorAll("sl-input, sl-select, sl-textarea").forEach(el => { try { el.value = ""; el.dispatchEvent(new Event("sl-change")); } catch { } }); update(); });
  form.addEventListener("submit", (e) => { e.preventDefault(); if (!form.checkValidity()) { form.reportValidity?.(); form.querySelector(":invalid")?.focus(); return; } console.log("Form submit data:", serializeForm(form)); });
  update();
}
(function bootForms() { document.querySelectorAll("form.demo-form").forEach(attachFormDemo); })();

// --- Shoelace: Log all htmx requests (for debug/dev)
if (window.htmx?.defineExtension) {
  window.htmx.defineExtension('logRequests', { onEvent(name, evt) { if (name === 'htmx:configRequest') { console.log('Request to:', evt.detail.path); } } });
}
